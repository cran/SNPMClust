snpmclust <-
function(indata,p=1,priorfrac=.2,uncertcutoff=.01,showplots=FALSE,
...
if (sum(!is.na(R))==0 | sum(!is.na(theta))==0) {
  outset <- data.frame(indata$SNP[p],indata$SampleID,as.character(rep("NC",length(R))),as.numeric(rep(NA,length(R))))
...
if (dim(unique(snpdat))[1]>5) {
  ...
  classif[!is.na(xtrans) & !is.na(Rtrans)] <- snpmclust$classification[1:length(xtrans[!is.na(xtrans) & !is.na(Rtrans)])]
  uncert[!is.na(xtrans) & !is.na(Rtrans)] <- snpmclust$uncertainty[1:length(xtrans[!is.na(xtrans) & !is.na(Rtrans)])]
  classif2 <- classif
  ...
  callrate <- (table(classif2)["AA"]
    +table(classif2)["AB"]
    +table(classif2)["BB"])/length(classif2)
  if (showplots) {
    plot(snpmclust)
    plot(theta,R,type="n",
      main=paste("uncertainty cutoff ",uncertcutoff,"; call rate ",callrate,sep=""),
      xlab="log-ratio",ylab="R.trans")
  ...
  calltxt[classif2=="BB"] <- "BB"
    ...
    uncert <- rep(NA,length(theta))
}
outset <- data.frame(indata$SNP[p],indata$SampleID,as.character(calltxt),as.numeric(uncert))
names(outset) <- c("SNP","SampleID","MClustCalls","Uncertainty")
return(outset)
}
